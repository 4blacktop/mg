...
// all news
/* 
$arrXml = objectsIntoArray(simplexml_load_string(file_get_contents('http://www.moigorod.ru/uploads/rss/_headlines/680000/news-main.xml', false, $context)));
// $arrXml = objectsIntoArray(simplexml_load_string(file_get_contents('news-main.xml', false, $context)));
echo '<hr />' . round((microtime(true) - $mtime) * 1, 4) . "\t<strong>Parsing News</strong>..."; flush(); // replace Category in echo
foreach ($arrXml['channel']['item'] as $item) {
	...
	echo '<br />' . round((microtime(true) - $mtime) * 1, 4) . "\tChecking news/" . $filename . ".html\t" . ' URL: ' . $item["pdalink"] . ''; flush(); // replace Category in "checking "
	$path = "news/$filename.html";
	if (file_exists($path)) {
		echo "\tCached!"; flush();
	} else {
	echo "\tDownloading..."; flush();
	file_get_contents($item["pdalink"], false, $contentHTML);
	file_put_contents($path, $contentHTML);
	}
}
echo '<br />' . round((microtime(true) - $mtime) * 1, 4) . "\t<strong>Done!</strong>" . '<hr />'; flush();
...
// city events
$arrXml = objectsIntoArray(simplexml_load_string(file_get_contents('http://www.moigorod.ru/uploads/rss/_headlines/680000/events-all.xml', false, $context)));
print_r($arrXml['channel']['item']);
 */
...
// cinema today
$arrXml = objectsIntoArray(simplexml_load_string(file_get_contents('http://www.moigorod.ru/uploads/rss/_headlines/680000/cinema-newfilms.xml', false, $context)));
// print_r($arrXml['channel']['item']);
print_r($arrXml);
echo '<hr />' . round((microtime(true) - $mtime) * 1, 4) . "\t<strong>Parsing Cinema RSS</strong>..." . count($arrXml) . " bytes"; flush(); // replace Category in echo
foreach ($arrXml['channel']['item'] as $item) {
	if (file_exists("stop.txt")) {exit("<br />stop.txt!");}
